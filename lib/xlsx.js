...
var xmlParser = require('xml2json-issue-37');
var fs = require('fs');
...
XLSX.prototype._init = function(cb) {
  ...
  var rs = this._readStream || fs.createReadStream(this._filePath);
  var rs = rs.pipe(unzip.Parse());
  rs.on('error', callback);
  rs.on('entry', this._bypassEntry.bind(this));
  rs.on('close', function() {
    debug('onClose picking:%d', self._picking);
    if (self._picking === 0) {
      self._initAll(cb);
    } else {
...
XLSX.prototype._bypassEntry = function(entry) {
  ...
  debug('Found entry: %s %s', entry.type, entry.path);
...
XLSX.prototype._setRow = function(sheetId, row) {
  ...
  if (row.r in this._rows) {
    var r = this._rows[row.r];
    rows[r] = row;
  } else {
    this._rows[row.r] = rows.length;
    rows.push(row);
  }
}
